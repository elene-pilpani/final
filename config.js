...
configOptions.forEach(e => 
	config[e] = Object.keys(savedConfig).includes(e) ? savedConfig[e]  : defaultConfig[e]
)
...
class Tester {
    ...
    getLastPostElem() {
        return this.getPostsFeed()
        .querySelector(`div.${testConfig.post}`)
    }
    ...
    getLastCommentElem() {
        return this.getCommentsFeed()
        .querySelector(`div.${testConfig.comment}`)
    }

    getCommentDeleteButton() {
        return this.getLastCommentElem()
            .querySelector(`button.${testConfig.commentDeleteButton}`)
    }
    ...
    getPostLikes() {
        ...
        const likesElem = elem.querySelector(`div.${testConfig.postLikesContainer}`)
        return {
            likes: elem,
    ...
    likePost() {
        const elem = this.getFirstPost()
        const likeButton = elem.querySelector(`button.${testConfig.postLikeButton}`)
        likeButton.onclick()
        return timeOutPromise()
    }
    register(params) {
        document.getElementById(testConfig.usernameInput).value = params.username
        document.getElementById(testConfig.passwordInput).value = params.password
        document.getElementById(testConfig.passwordRepeatInput).value = params.repeatPassword || params.password
